...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!study) return {};
  return pageMetadata(`/case-studies/${slug}`, study.subtitle, study.summary);
}

export default async function CaseStudyPage({ params }: Props) {
  const { slug } = await params;
  const [CASE_STUDIES, m] = await Promise.all([getCaseStudies(), getMessages()]);
  ...
  const sections = [
    { label: t.challenge, body: study.challenge },
    { label: t.delivered, body: study.delivered },
    { label: t.solution, body: study.solution },
  ];
    ...
    <PageShell>
      <PageHero eyebrow={study.sector} titleLines={[study.name]} sub={study.subtitle}>
        <dl className="mt-14 grid gap-8 border-t border-white/20 pt-8 sm:grid-cols-2">
          <div className="flex flex-col gap-1">
            <dt className="text-[12px] font-semibold uppercase tracking-widest text-white/60">{t.industry}</dt>
            <dd className="text-[16px] font-medium text-white">{study.industry}</dd>
          </div>
          <div className="flex flex-col gap-1">
            <dt className="text-[12px] font-semibold uppercase tracking-widest text-white/60">{t.focus}</dt>
      ...
      <section className="bg-ink">
        <div className="container-x flex flex-col gap-10 [padding-block:120px] lg:flex-row lg:gap-20">
          <Reveal className="lg:w-[320px] lg:shrink-0">
            <SectionLabel dark>{t.results}</SectionLabel>
          </Reveal>
          <Reveal delay={80}>
      ...
      <section className="bg-white">
        ...
        >
          ...
          <div className="flex items-end justify-between gap-6">
            ...
            <span className="shrink-0 font-display text-[clamp(30px,5vw,72px)] leading-[0.9] text-accent transition-transform duration-500 group-hover:translate-x-3 rtl:-scale-x-100 rtl:group-hover:-translate-x-3">
              →
            </span>
          </div>
