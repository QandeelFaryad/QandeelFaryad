...
export async function Capabilities({ dark = false }: { dark?: boolean }) {
  const t = (await getMessages()).capabilities;
  return (
        ...
        <div className="flex flex-col gap-6">
          <Reveal>
            <div className="flex flex-wrap items-center gap-4">
              <SectionLabel dark={dark}>{t.label}</SectionLabel>
              <span className="font-display text-[13px] font-bold uppercase text-spark">{fmt(t.count, { n: t.items.length })}</span>
            </div>
          </Reveal>
          <Reveal delay={80}>
            ...
            <Reveal key={c.title} delay={(i % 3) * 80} scale>
              <Tilt max={4}>
                <div className={`flex h-full flex-col gap-3 rounded-3xl p-8 ${dark ? "bg-white/[0.04]" : "bg-white"}`}>
                  <span className="font-display text-[18px] font-bold text-spark">{String(i + 1).padStart(2, "0")}</span>
